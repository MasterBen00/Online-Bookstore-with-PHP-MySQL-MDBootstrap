...
<main>
    ...
    <section class="team-section my-5 ">
        ...
        <!-- Section heading -->
        <hr class="my-5">
        <h2 class="h1-responsive font-weight-bold text-center my-5">Sold Books</h2>
        <p class="grey-text text-center w-responsive mx-auto mb-5">Your Sold Books</p>
        <div class="row text-center text-md-left">

            <?php
            $conn=db_connect();

            $sql = "SELECT distinct book_id FROM order_book where confirm=2";
            $result6 = mysqli_query($conn, $sql);
            while($row6=mysqli_fetch_array($result6)) {
                $bid = $row6['book_id'];

                $query = "SELECT * from book_admin where book_id='$bid'";
      ...
      <div class=\"avatar mb-md-0 mb-4 mx-4\">
        ...
        <p class=\"grey-text\">" . $row1[0] . "</p>
							  ...
							  <a class=\"btn btn-deep-purple btn-rounded btn-sm\" href='admin_history_book_detail.php?book_id=" . $row['book_id'] . "'>More</a>
							  <a class=\"btn btn-unique btn-rounded btn-sm\" href='admin_delete_permanent.php?book_id=" . $row['book_id'] . "'>Del</a>
